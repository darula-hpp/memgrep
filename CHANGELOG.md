...
- **`npm start` / `npm stop`** — `scripts/start-all.sh` builds if needed, ensures `~/.memgrep/mcp-token`, reinstalls Telegram (`--all`) + jobs LaunchAgents (with `MEMGREP_MCP_TOKEN` in the plist), and starts the ngrok HTTPS tunnel. `npm stop` tears them down.
- **ngrok Host allowlist** — HTTP MCP accepts tunnel hostnames via `MEMGREP_NGROK_DOMAIN` / `MEMGREP_ALLOWED_HOSTS` / `~/.memgrep/mcp-public-url` (fixes `Invalid Host` 403 behind ngrok). `npm start` requires the domain from env/`.env` or that url file (no hardcoded personal ngrok hostname in the repo).
- **Cursor agent MCP tools** — optional suite on the existing memgrep MCP server (`cursor_workspaces`, `cursor_status`, `cursor_run`) that drives a **local** `@cursor/sdk` agent with a workspace allowlist. Configure with `node dist/cli.js cursor setup` (`~/.memgrep/cursor.json` or `CURSOR_API_KEY` + Telegram profile workspaces). Shared `runAgentTurn` runner is also used by Telegram and jobs. For remote/cloud Cursor clients: `memgrep serve --http` + `MEMGREP_MCP_TOKEN` + ngrok.
